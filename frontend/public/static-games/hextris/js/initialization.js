...
function initialize(a) {
	...
	// Load best score from GameSDK
	if (window.GameSDK && window.GameSDK.loadBestScore) {
		GameSDK.loadBestScore().then(function (score) {
			if (score && score > 0) {
				// If SDK has a better score, use it
				if (highscores.length === 0 || score > highscores[0]) {
					highscores = [score].concat(highscores.filter(function (s) { return s !== score; }));
			...
			// Silent fail - will use local scores
		});
	}
	window.blocks = [];
	window.MainHex;
